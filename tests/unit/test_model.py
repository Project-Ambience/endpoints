...
def test_install_model_success(mock_post, mock_snapshot):
    install_model_and_notify(
        "sshleifer/tiny-distilroberta-base",
        "http://fake/callback",
        "id-123",
    )
    mock_snapshot.assert_called_once()
    mock_post.assert_called_once_with("http://fake/callback", json={"id": "id-123", "status": "success"})
...
def test_install_model_failure(mock_post, mock_snapshot):
    ...
    mock_snapshot.assert_called_once()
    mock_post.assert_called_once_with("http://fake/callback", json={"id": "id-999", "status": "fail"})
